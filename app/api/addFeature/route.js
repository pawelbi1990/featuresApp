...
export async function POST(request, res) {
  ...
  const queryLoop = async (array, client, imageFile, clientId, clientName) => {
    for (let i = 0; i < array.length; i++) {
      ...
      const values = [
        frontTaskData.name,
        frontTaskData.short_desc,
        frontTaskData.long_desc,
        frontTaskData.assigned,
        clientId[i],
        clientName[i],
      ];
  ...
  const mappedArrayTextAfterDot = await mappedArray.map((value) =>
  ...
  if (auth === false) {
    ...
    result = await queryLoop(
      mappedArray,
      client,
      frontTaskData.image_file,
      clientIdArray,
      mappedArrayTextAfterDot
    );
  }
